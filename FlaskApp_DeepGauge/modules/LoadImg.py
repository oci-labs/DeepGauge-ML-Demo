...
class Dataset(object):
    ...
    @staticmethod
    def dump_pkl_MultiColProcessor(gauge_categories):
        ...
        with open('dumps/MultiColProcessor.pkl', 'wb') as pklFile:
            pickle.dump(MultiColumnOneHotEncoder, pklFile)
        return

    @staticmethod
    def dump_json_category_mapper(data):
        ##
    ...
    def load_dataset(path):
        ...
        Dataset.dump_json_category_mapper(data=data)
        ...
        ##
        Dataset.dump_pkl_MultiColProcessor(gauge_categories=gauge_categories)

        return guage_files, gauge_categories

    @staticmethod
    def split_data_files(ver_ratio, path, random_state):
        ##
        guage_files, gauge_categories = Dataset.load_dataset(path=path)
    ...
    def prep_datasets(cls, ver_ratio, container_path, final_img_width, final_img_height,
        ...
        ##
        X_train_path_names, X_test_path_names, y_train, y_test = \
            cls.split_data_files(ver_ratio=ver_ratio, path=container_path, random_state=random_state)
